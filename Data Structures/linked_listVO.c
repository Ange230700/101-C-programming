...
int main()
{
	...
	while (input != 0)
	{
		...
		switch (input)
		{
			...
			case 2:
				print("Your choice: 'Deletion'\n");
				print("Enter the value which should be deleted: ");
				scanf("%d", &num);
...
void append_node(struct NODE *llist, int num)
{
	...
	llist -> next = (struct NODE *) malloc(size of(struct NODE));
...
int search_valued(struct NODE *llist, int num)
{
	int retval = -1;
	int i = 1;

	while (llist -> next != num)
	{
